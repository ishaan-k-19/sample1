...
/* ─── Desktop: original pinned clip-path animation ───────────────────────────── */
...
const DesktopProducts = () => {
  ...
  useGSAP(
    () => {
      if (window.innerWidth < 768) return // mobile uses MobileProducts
  ...
  return (
    ...
    >
        ...
        return (
          ...
          >
            <div className="relative z-10 w-1/2 h-full">
              <div className="slide-content relative h-full flex flex-col justify-end px-10 lg:px-[6vw] pb-16 lg:pb-[8vw]" style={{ opacity: isFirst ? 1 : 0 }}>
                <h2 className="mb-[2vw]">
                  <div className="overflow-hidden">
                    <span className="block font-display text-[11vw] text-white leading-[0.9]">
                      {first}
                    </span>
                  </div>
                  <div className="overflow-hidden" style={{ marginTop: '-0.8vw' }}>
                    <span className={`block font-display text-[11vw] leading-[0.9] ${product.color}`}>
                      {last}
                    </span>
                  </div>
                </h2>
                ...
                <div className="flex flex-col lg:flex-row lg:items-end gap-6 lg:gap-10">
                  <div className="flex-1">
                    <p className="font-body text-xs tracking-[0.2em] text-white/80 uppercase mb-3">
                      {product.flavor}
                    </p>
                    <div className={`w-10 h-[1.5px] bg-gradient-to-r ${product.gradient} mb-4`} />
                    <p className="font-body text-sm text-white/85 leading-relaxed max-w-xs lg:max-w-[22vw] mb-5">
                      {product.description}
                    </p>
                    <div className="flex flex-wrap gap-2 lg:mb-0">
                      {['20g Protein', '0 Sugar', '0 Carbs', '0 Fats'].map((tag, j) => (
                        <span
                          key={j}
                          className="px-3 py-1 border border-white/30 rounded-full font-body text-[10px] text-white/80 tracking-wide"
                        >
                          {tag}
                        </span>
                      ))}
            ...
            <div className="slide-image relative w-1/2 overflow-hidden">
              <div
              ...
              />
              <div className="absolute inset-0 flex items-center justify-center product-img-wrap">
                <Image
                  src={product.image}
                  alt={`${product.name} KLIQ Energy Drink`}
                  width={400}
                  height={600}
                  className="h-[70vh] w-auto object-contain max-w-none drop-shadow-[0_25px_50px_rgba(0,0,0,0.5)]"
                  style={{
      ...
      <div className="absolute right-6 top-1/2 -translate-y-1/2 flex flex-col items-center gap-3 z-50 pointer-events-none">
...
/* ─── Mobile: yPercent slide-up reveal ────────────────────────────────────────── */
...
const MobileProducts = () => {
  ...
  useGSAP(
    () => {
      ...
      // Pre-promote all slides to GPU layers
      slides.forEach((slide, i) => {
        gsap.set(slide, { yPercent: i === 0 ? 0 : 100, force3D: true })
      })
      slideContent.forEach(el => {
        if (el) gsap.set(el, { force3D: true })
      })

      // Build a scrubbed timeline instead of onUpdate for yPercent
      // This lets GSAP handle interpolation natively (fewer JS calls per frame)
      ...
      // For each transition: slide up the next slide, crossfade content
      for (let i = 0; i < numTransitions; i++) {
        ...
        if (slideContent[i]) {
          tl.to(slideContent[i], {
            opacity: 0, duration: 0.4, ease: 'power2.in', force3D: true,
          }, t)
        }

        // Fade in next content
        if (slideContent[i + 1]) {
          tl.fromTo(slideContent[i + 1],
            { opacity: 0 },
  ...
  return (
    ...
    >
        ...
        return (
          ...
          >
            <div className="relative z-10 w-full h-full">
              <div className="slide-content relative h-full flex flex-col justify-end px-6 pt-[30%] pb-[30%]" style={{ opacity: isFirst ? 1 : 0 }}>
                <h2 className="mb-4">
                  <div className="overflow-hidden">
                    <span className="block font-display text-[14vw] text-white leading-[0.9]">
                      {first}
                    </span>
                  </div>
                  <div className="overflow-hidden" style={{ marginTop: '-0.8vw' }}>
                    <span className={`block font-display text-[14vw] leading-[0.9] ${product.color}`}>
                      {last}
                    </span>
                  </div>
                </h2>
                ...
                <div className="flex-1 flex flex-col justify-end">
                  <p className="font-body text-xs tracking-[0.2em] text-white/80 uppercase mb-2">
                    {product.flavor}
                  </p>
                  <div className={`w-10 h-[1.5px] bg-gradient-to-r ${product.gradient} mb-3`} />
                  <p className="font-body text-xs text-white/85 leading-relaxed max-w-xs mb-4">
                    {product.description}
                  </p>
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {['20g Protein', '0 Sugar', '0 Carbs', '0 Fats'].map((tag, j) => (
                      <span
                        key={j}
                        className="px-2.5 py-1 border border-white/30 rounded-full font-body text-[9px] text-white/80 tracking-wide"
                      >
                        {tag}
                      </span>
                    ))}
            ...
            <div className="slide-image absolute inset-0 overflow-hidden">
              <div
                className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[250px] h-[250px] rounded-full blur-[40px] pointer-events-none"
                style={{ background: `rgba(${product.glowColor}, 0.15)` }}
              />
              <div className="absolute inset-0 flex items-start pt-[50%] justify-center pl-[20%] pr-[5%] product-img-wrap">
                <Image
                  src={product.image}
                  alt={`${product.name} KLIQ Energy Drink`}
                  width={400}
                  height={600}
                  className="h-[40vh] w-auto object-contain max-w-none"
                  style={{ transform: isFirst ? 'scale(1)' : 'scale(0.85)' }}
                  sizes="80vw"
                  priority={i === 0}
                />
              </div>
      ...
      <div className="absolute right-4 top-1/2 -translate-y-1/2 flex flex-col items-center gap-3 z-50 pointer-events-none">
        ...
        <span className="font-body text-[9px] tracking-widest text-white/60 uppercase">
          Scroll
        </span>
      </div>
...
/* ─── Wrapper: both render, CSS toggles visibility ───────────────────────────── */
